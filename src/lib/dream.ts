...
- "graph_updates": object with two fields:
    ...
    - "relations": array of {fromName, toName, type} objects describing how entities relate. "type" should be a lowercase underscore string. Prefer these established types where they fit:
      ...
      If none fit, coin a new type in the same lowercase underscore style. Always use active direction: fromName is the actor, toName is the object. Never use passive forms — flip the entities instead. E.g. instead of (Being, "is_governed_by", Ethics) write (Ethics, "governs", Being). Only include relations that are factual and stable.
